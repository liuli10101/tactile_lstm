...
# 1 Δforce feature (变体1：启用该函数，添加动态特征)
...
# 4 Model (变体1：仅加入Δforce，其余与基线一致)
class SlipDetectionModel(nn.Module):
    def __init__(self):
        super().__init__()
        # ========== 变体1修改：输入维度改为624（原始312+Δforce312） ==========
        self.feature_proj = nn.Linear(624, 128)
        self.relu = nn.ReLU()
        
        # 保持与基线一致：单向LSTM（bidirectional=False）
        self.lstm = nn.LSTM(
            input_size=128,
            hidden_size=64,
            num_layers=2,
            batch_first=True,
            bidirectional=False  # 单向，与基线一致
        )
        
        # 保持与基线一致：去掉Attention层，用全局平均池化
        self.dropout = nn.Dropout(0.3)
        
        # 保持与基线一致：LSTM输出维度64（单向），FC层输入不变
        self.fc1 = nn.Linear(64, 64)
        self.fc2 = nn.Linear(64, 1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # x (B,T,624) - 输入维度变为624（原始+Δforce）
        x = self.feature_proj(x)
        x = self.relu(x)
        
        lstm_out, _ = self.lstm(x)  # lstm_out shape: (B, T, 64) - 单向，与基线一致
        
        # 保持与基线一致：全局平均池化（无Attention）
...
def train_model(model, train_loader, val_loader, device, save_path, epochs=30, patience=5):
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            preds = (outputs > 0.25).float()  # 与基线保持相同阈值
...
# ==================== 6 Evaluation (English Version，仅修改标题区分变体1) ====================
...
def evaluate_full_model(model, val_loader, device, mean, std, model_save_path, output_dir):
    ...
    print("\n" + "="*50)
    print("Variant 1 (w/ Δforce) Full Evaluation Results")  # 标注变体1
    print("="*50)
    ...
    # 文件名标注变体1，避免与其他模型混淆
    cm_path = os.path.join(output_dir, 'confusion_matrix_variant1.png')
    plot_confusion_matrix(y_true_all, y_pred_all, classes=class_names, save_path=cm_path, title='Variant 1 (w/ Δforce) Confusion Matrix')

    precision_curve, recall_curve, _ = precision_recall_curve(y_true_all, y_score_all)
    # 文件名标注变体1
    pr_path = os.path.join(output_dir, 'pr_curve_variant1.png')
    ...
    plt.title('Variant 1 (w/ Δforce) Precision-Recall Curve', fontsize=14)  # 标注变体1
    plt.ylim([0.0, 1.05])
    plt.xlim([0.0, 1.0])
...
# 7 Main (核心修改：启用Δforce，修改保存路径/名称区分变体1)
if __name__ == "__main__":
    # ==================== Path Config (修改名称，明确区分变体1) ====================
    data_folder = "/home/liuli/tactile_lstm/train_data/data306"
    model_save_dir = "/home/liuli/tactile_lstm/models"
    eval_output_dir = "/home/liuli/tactile_lstm/eval_results/eval_results_variant1"  # 变体1单独文件夹
    os.makedirs(model_save_dir, exist_ok=True)
    
    model_name = "lstm_variant1_deltaforce"  # 标注变体1+Δforce
    ...
    # ========== 变体1核心修改：启用Δforce特征添加 ==========
    X_train = add_delta_feature(X_train)
    X_val = add_delta_feature(X_val)
    print("Variant 1: Using original 312-dim + Δforce 312-dim features (Total 624-dim)")
    ...
    # ==================== Training ====================
    print("\n" + "="*50)
    print("Starting Variant 1 (w/ Δforce) Training")  # 标注变体1
    print("="*50)
    best_f1 = train_model(model, train_loader, val_loader, device, model_save_path, epochs=30, patience=5)

    # ==================== Save Full Model ====================
    print(f"\nSaving full Variant 1 model (with norm params) to: {final_save_path}")
    torch.save({
    ...
    # ==================== Full Evaluation ====================
    print("\n" + "="*50)
    print("Starting Variant 1 (w/ Δforce) Model Evaluation")  # 标注变体1
    print("="*50)
    evaluate_full_model(
        model=model,
    ...
    print("\nAll Variant 1 (w/ Δforce) processes completed!")
