...
# 1 Δforce feature (保留函数但不调用，用于对比)
...
class SlipDetectionModel(nn.Module):
    def __init__(self):
        super().__init__()
        # ========== 修改1：Baseline - 输入维度改为312（仅原始特征） ==========
        self.feature_proj = nn.Linear(312, 128)
        self.relu = nn.ReLU()
        
        # ========== 修改2：Baseline - 使用单向LSTM（bidirectional=False） ==========
        self.lstm = nn.LSTM(
            input_size=128,
            hidden_size=64,
            num_layers=2,
        ...
        # ========== 修改3：Baseline - 去掉Attention层 ==========
        # self.attn = nn.Linear(128, 1)  # 删除Attention
        
        self.dropout = nn.Dropout(0.3)
        
        # ========== 修改4：Baseline - LSTM输出维度变为64（单向），调整FC层输入 ==========
    ...
    def forward(self, x):
        ...
        lstm_out, _ = self.lstm(x)  # lstm_out shape: (B, T, 64) - 单向
        
        # ========== 修改5：Baseline - 用全局平均池化替换Attention ==========
        # weights = torch.softmax(self.attn(lstm_out), dim=1)
        # context = torch.sum(lstm_out * weights, dim=1)
        context = torch.mean(lstm_out, dim=1)  # 全局平均池化
...
def train_model(model, train_loader, val_loader, device, save_path, epochs=30, patience=5):
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            preds = (outputs > 0.25).float()  # 保持阈值一致，或改为0.5
...
def evaluate_full_model(model, val_loader, device, mean, std, model_save_path, output_dir):
    ...
    print("\n" + "="*50)
    print("Baseline Model Full Evaluation Results")
    print("="*50)
    ...
    plot_confusion_matrix(y_true_all, y_pred_all, classes=class_names, save_path=cm_path, title='Baseline Confusion Matrix')

    precision_curve, recall_curve, _ = precision_recall_curve(y_true_all, y_score_all)
    pr_path = os.path.join(output_dir, 'pr_curve_baseline.png')
...
if __name__ == "__main__":
    # ==================== Path Config (建议修改保存名以区分Baseline) ====================
    data_folder = "/home/liuli/tactile_lstm/train_data/data306"
    model_save_dir = "/home/liuli/tactile_lstm/models"
    eval_output_dir = "/home/liuli/tactile_lstm/eval_results/eval_results_baseline"  # 单独文件夹
    os.makedirs(model_save_dir, exist_ok=True)
    
    model_name = "lstm_baseline"  # 修改模型名
    ...
    # ========== 修改6：Baseline - 去掉Δforce特征添加 ==========
    # X_train = add_delta_feature(X_train)
    # X_val = add_delta_feature(X_val)
    print("Baseline: Using original 312-dim features (No Δforce)")
    ...
    # ==================== Save Full Model ====================
    print(f"\nSaving full baseline model (with norm params) to: {final_save_path}")
    torch.save({
    ...
    print("\nAll Baseline processes completed!")
